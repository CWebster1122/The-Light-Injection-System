...
int Fitter()
{
  Bool_t Write = 0; // Toggle writing to parameter files  *Note: Must be ON when running Run_Fitter.sh and OFF when checking individual root file*
  ...
  TCanvas *c2, *c3, *c5, *c6; // I have to define variables outside of booleans to make sure they are defined. This has caused some trouble in the past.
  ...
  TFile *f = new TFile("1519251813.root"); // "temp.root" when running Run_Fitter.sh
